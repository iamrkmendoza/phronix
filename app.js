...
if(window.location.pathname.endsWith('login.html')) {
  // Skip all app initialization on login page
} else {
  if(!localStorage.getItem('phronix_profile')){
...
/* === INIT === */
// Check if this is the user's absolute first time entering the app
if (!localStorage.getItem('phronix_first_time_check')) {
  // 1. Give them their starting reward XP
  profile.totalXp += 50; 
  profile.level = getLvl(profile.totalXp).level;
  
  // 2. NEW FIX: Add the 50 XP directly to today's bar slot on the progress chart
  const currentDayName = dayName(new Date().getDay()); // Detects if it's 'Sun', 'Mon', etc.
  if (weeklyXp[currentDayName] !== undefined) {
    weeklyXp[currentDayName] += 50;
  }
  
  // 3. Log it to recent activities
  recent.unshift({
    text: 'Journey Begun!',
  ...
  // 4. Save everything securely to storage tracking layers
  save('profile', profile);
  save('weeklyXp', weeklyXp); // Commits the chart bar data
  save('recent', recent);
  
  // 5. Create and launch the magical flame popup animation
